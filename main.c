...
#define kk  9           /* kk = nn-2*tt  */ // k - liczba symboli wiadomoœci oryginalnej; 2t = n - k - liczba jednostek parzystoœci
...
*/
 {
  ...
  for (i=0; i<mm; i++)
   { alpha_to[i] = mask ;
     ...
     //printf("Index:\n index_of[alpha_to[i]]: %d\n\n", index_of[alpha_to[i]]);
     if (pp[i]!=0){
       //printf("Przed xorem:\n mask: %d\n alfa[mm]: %d\n\n", mask, alpha_to[mm]);
       alpha_to[mm] ^= mask ; //a = a ^ mask - operator bitowy xor
       //printf("Po xorze:\n mask: %d\n alfa[mm]: %d\n\n", mask, alpha_to[mm]);
     }
     mask <<= 1 ;
     //printf("Wartosci zmiennych po przesunieciu bitowym:\n mask: %d\n alfa[i]: %d\n alfa[mm]: %d\n\n\n\n", mask, alpha_to[i], alpha_to[mm]);
   }

  //printf("Wartości zmiennych: %d, %d, %d", mask, alpha_to[i], alpha_to[mm]);
...
*/
 {
   ...
   for (i=0; i<=nn-kk; i++) {  gg[i] = index_of[gg[i]]; printf("Forma indeksowa:\n gg[i]: %d\n", gg[i]);
   }
    for (i=0; i<=14; i++)
    {
        printf("Indeks %d, %d\n", i, index_of[i]);
...
/* take the string of symbols in data[i], i=0..(k-1) and encode systematically
   ...
   Codeword is   c(X) = data(X)*X**(nn-kk)+ b(X)          */
 {
   ...
   int feedback ;
    ...
    // Przejscie po petlach, sprawdzenie wartosci parametrow
   for (i=0; i<nn-kk; i++){
    bb[i] = 0 ;
   }
   for (i=kk-1; i>=0; i--)
       ...
       if (feedback != -1)
        { for (j=nn-kk-1; j>0; j--){
            if (gg[j] != -1)
              ...
              bb[j] = bb[j-1] ;
            printf("Tablica z bitami parzystosci [%d] %d\n", j, bb[j]);}
